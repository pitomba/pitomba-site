...
def setup():
    ...
    put(os.path.join(LOCAL_DIR, 'deploy', 'upstart.conf'),
        '/etc/init/pitomba.conf', use_sudo=True)
    put(os.path.join(LOCAL_DIR, 'deploy', 'nginx-site.conf'),
        '/etc/nginx/sites-enabled/pitomba.conf', use_sudo=True)
...
def deploy():
    ...
    # move settings
    with cd("{}pitomba/".format(PROJECT_DIR)):
